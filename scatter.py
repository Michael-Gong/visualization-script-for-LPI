...
def main(from_path, to_path):
  for n in range(start,stop+step,step):
    #### header data ####
    data0 = sdf.read("./epoch2dno/"+from_path+str(n).zfill(4)+".sdf",dict=True)
    data1 = sdf.read("./epoch2drr/"+from_path+str(n).zfill(4)+".sdf",dict=True)
    data2 = sdf.read("./epoch2dqe/"+from_path+str(n).zfill(4)+".sdf",dict=True)
    header=data0['Header']
    time=header['time']
