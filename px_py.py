...
def main(from_path, to_path):
  for n in range(start,stop+step,step):
    ...
    plt.scatter(py/a0,px/(a0**2)*2,s=0.5,c=(0.0,0.0,192.0/255.0),label=r'dx=1/20$\lambda; a_0=$'+str(a0),edgecolors='None')
    ...
    py    = data['Particles/Py/electron'].data/(m0*v0)
    gg    = np.sqrt(px**2+py**2+1.0)
    plt.scatter(py/a0,px/(a0**2)*2,s=0.5,c=(0.0,192.0/255.0,0.0),label=r'dx=1/40$\lambda; a_0=$'+str(a0),edgecolors='None')
    ...
    py    = data['Particles/Py/electron'].data/(m0*v0)
    gg    = np.sqrt(px**2+py**2+1.0)
    plt.scatter(py/a0,px/(a0**2)*2,s=0.5,c=(192.0/255.0,0.0,0.0),label=r'dx=1/80$\lambda; a_0=$'+str(a0),edgecolors='None')
    ...
    #ax.set_rticks(20);
    plt.title(r'$p_x-p_y$ at '+str(round(time/3.3333e-15,1))+' $T_0$',fontdict=font)
    ...
    plt.scatter(x,gg-px,s=2.0,c=(0.0,0.0,192.0/255.0),label=r'dx=1/20$\lambda; a_0=$'+str(a0),edgecolors='None')
    data = sdf.read('Data40/'+str(n).zfill(4)+".sdf",dict=True)
    header=data['Header']
    time=header['time']
    px    = data['Particles/Px/electron'].data/(m0*v0)
    ...
    gg    = np.sqrt(px**2+py**2+1.0)
    x     = data['Grid/Particles/electron'].data[0]/1.0e-6
    plt.scatter(x,gg-px,s=2.0,c=(0.0,192.0/255.0,0.0),label=r'dx=1/40$\lambda; a_0=$'+str(a0),edgecolors='None')
    data = sdf.read('Data80/'+str(n).zfill(4)+".sdf",dict=True)
    header=data['Header']
    time=header['time']
    px    = data['Particles/Px/electron'].data/(m0*v0)
    ...
    gg    = np.sqrt(px**2+py**2+1.0)
    x     = data['Grid/Particles/electron'].data[0]/1.0e-6
    plt.scatter(x,gg-px,s=2.0,c=(192.0/255.0,0.0,0.0),label=r'dx=1/80$\lambda; a_0=$'+str(a0),edgecolors='None')
    ...
    #ax.set_rticks(20);
    plt.title(r'$\gamma-px vs x$ at '+str(round(time/3.3333e-15,1))+' $T_0$',fontdict=font)
    #ax.set_title(r"(a) $\xi_0=250$", va='bottom', fontdict=font)
    #plt.legend(loc='upper center',framealpha=0.0,markerscale=10.0,fontsize=20.0,scatterpoints=3)

    fig = plt.gcf()
    fig.set_size_inches(10,16)
    fig.savefig(to_path+'px_py'+str(n).zfill(4)+'.png',format='png',dpi=100)
    plt.close("all")
