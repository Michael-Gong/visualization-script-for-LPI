...
y1 = -(fieldex)[index,start:]
y2 = np.zeros_like(y1)
ax.fill_between(xx[index,start:],y1,y2, where=y1>=y2, facecolor='green',alpha=0.3,interpolate=True)
ax.fill_between(xx[index,start:],y1,y2, where=y1<=y2, facecolor='red',alpha=0.3,interpolate=True)
plt.scatter(xx[index,:], yy[ind, c=gg[index,:], norm=norm_x, s=6, cmap='autumn', edgecolors='None')



ax = plt.subplot(5,2,5)
y1 = -(fieldey*py/px)[index,start:]
y2 = np.zeros_like(y1)
ax.fill_between(xx[index,start:],y1,y2, where=y1>=y2, facecolor='green',alpha=0.3,interpolate=True)
ax.fill_between(xx[index,start:],y1,y2, where=y1<=y2, facecolor='red',alpha=0.3,interpolate=True)

ax = plt.subplot(5,2,7)
y1 = -(fieldey*py/px)[index,start:]-(fieldex)[index,start:]
y2 = np.zeros_like(y1)
ax.fill_between(xx[index,start:],y1,y2, where=y1>=y2, facecolor='green',alpha=0.3,interpolate=True)
ax.fill_between(xx[index,start:],y1,y2, where=y1<=y2, facecolor='red',alpha=0.3,interpolate=True)
...
norm_x = matplotlib.colors.Normalize(vmin=1.,vmax=301.)
...
plt.scatter(xx[index,:],yy[index,:], c=gg[index,:], norm=norm_x, s=6, cmap='winter', edgecolors='None')
...
plt.scatter(xx[index,:],gg[index,:], c=gg[index,:], norm=norm_x, s=6, cmap='winter', edgecolors='None')
...
fig = plt.gcf()
fig.set_size_inches(32.0, 25)
fig.savefig('./figure_wrap_up/'+'trajectory_pwer.png',format='png',dpi=160)
plt.close("all")
