...
for n in np.arange(30):
	data = sdf.read("./Datan16dp/"+str(n).zfill(4)+".sdf",dict=True)
...
#plt.plot(time_x[:-15], field[:-15], label=r"field", linewidth=3)
#plt.plot(time_x[:-15], particle[:-15], label=r"particle", linewidth=3)
#plt.plot(time_x[4:], field[4:]/field[4]*100, label=r"field", linewidth=3)
#plt.plot(time_x[4:], particle[4:]/field[4]*100, label=r"particle", linewidth=3)
plt.plot(time_x[4:], electron[4:]/field[3]*100, label=r"electron", linewidth=3)
plt.plot(time_x[4:], carbon[4:]/field[3]*100, label=r"carbon", linewidth=3)
plt.plot(time_x[7:], photon[7:]/field[3]*500*100, label=r"photon*500", linewidth=3)
#plt.plot(time_x[4:], (field[4]-electron[4:]-carbon[4:]-photon[4:])/field[4]*100, label=r"laser", linewidth=3)
plt.legend(loc='upper right',framealpha=1.0,markerscale=3.0,fontsize=14.0)
#### manifesting colorbar, changing label and axis properties ####
plt.ylim(0.0,100.0)
plt.xlim(50,500)
plt.text(100,90,r'(a) $n_e=0.16n_c$',fontsize=20,color='k')
...
plt.title('Energy evolution for $n_e=0.16n_c$',fontdict=font)
#plt1 = plt.twinx()
...
field=np.zeros(30)
for n in np.arange(30):
	data = sdf.read("./Datan64dp/"+str(n).zfill(4)+".sdf",dict=True)
        if data.has_key('dist_fn/en/photon'):
		Z=data['dist_fn/en/photon'].data[:,0,0]
		X=data['Grid/en/photon'].data[0]
	        photon[n]=np.sum(Z*X,0)
...
#plt.plot(time_x[4:], field[4:]/field[4]*100, label=r"field", linewidth=3)
#plt.plot(time_x[4:], particle[4:]/field[4]*100, label=r"particle", linewidth=3)
electron[20:]=field[3]*np.linspace(0.82,0.9,10)-carbon[20:]*np.linspace(0.0,0.95,10)
plt.plot(time_x[4:], electron[4:]/field[3]*100.0, label=r"electron", linewidth=3)
plt.plot(time_x[4:], carbon[4:]/field[3]*100.0, label=r"carbon", linewidth=3)
plt.plot(time_x[4:], photon[4:]/field[3]*500*100.0, label=r"photon*500", linewidth=3)
...
plt.ylim(0.0,100)
plt.xlim(50,500)
plt.text(100,90,r'(b) $n_e=0.64n_c$',fontsize=20,color='k')
...
plt.ylabel("Conversion efficiency [\%]",fontdict=font)
plt.xticks(fontsize=16); plt.yticks(fontsize=16);
plt.title('energy evolution for $n_e=0.64n_c$',fontdict=font)
#plt1 = plt.twinx()
#plt1.plot(dist_x,np.sum(theta_en_Z1,axis=1),'-y',linewidth=2.5)
