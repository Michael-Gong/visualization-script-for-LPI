...
plt.scatter(xx[index,:], yy[index,:], c=gg[index,:], norm=norm_y, s=6, cmap='winter', edgecolors='None')
...
plt.plot(np.linspace(0,130,1000),np.zeros_like(np.linspace(0,130,1000)),linewidth=0.5,linestyle=':',color='k')
...
plt.scatter(xx[index,:], y1, c=gg[index,:], norm=norm_y, s=6, cmap='winter', edgecolors='None')
plt.xticks([],fontsize=20); plt.yticks([-5,0,5],fontsize=20);
#plt.ylabel(r'$\frac{dW_x}{dx}$ [m$_e$c$\omega_0$]',fontdict=font)
plt.ylabel(r'dW$_x$/dx',fontdict=font)
plt.xlim(4,126)
plt.ylim(-1.2*2.*np.pi,1.8*2.*np.pi)
...
ax.fill_between(xx[index,:],y1,y2, where=y1<=y2, facecolor='red',alpha=0.3,interpolate=True)
plt.scatter(xx[index,:], y1, c=gg[index,:], norm=norm_y, s=6, cmap='winter', edgecolors='None')
plt.xticks([],fontsize=20); plt.yticks([-5,0,5,10],fontsize=20);
#plt.ylabel(r'$\frac{dW_{x+y}}{dx}$ [m$_e$c$\omega_0$]',fontdict=font)
plt.ylabel('dW$_t$/dx',fontdict=font)
plt.xlim(4,126)
plt.xlim(4,126)
plt.ylim(-1.6*2.*np.pi,2.8*2.*np.pi)


ax = plt.subplot(5,1,5) 
plt.scatter(xx[index,:], gg[index,:], c=gg[index,:], norm=norm_y, s=6, cmap='winter', edgecolors='None')
...
plt.xticks([20,40,60,80,100,120],fontsize=20); plt.yticks([0,200,400],fontsize=20);
...
fig = plt.gcf()
fig.set_size_inches(10.0, 9.5)
fig.savefig('./figure_wrap_up/'+'trajectory_power_y_1.png',format='png',dpi=160)
plt.close("all")
